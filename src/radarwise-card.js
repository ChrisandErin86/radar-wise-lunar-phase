...
const CARD_TYPES = ["radarwise-lunar-card", "radar-wise-lunar-card", "weatherwise-lunar-card", "weather-wise-lunar-card"];
const NOAA_RADAR_WMS = "https://opengeo.ncep.noaa.gov/geoserver/wms";
...
const RADARWISE_THEME_MODES = {
  radarwise: "RadarWise",
  dark: "RadarWise Dark",
  auto: "Match Home Assistant theme"
};

const RADARWISE_FORECAST_FEATURES = {
  daily: 1,
  hourly: 2,
  twice_daily: 4
};

const RADARWISE_FORECAST_TYPES = ["hourly", "daily", "twice_daily"];
...
class RadarWiseCard extends HTMLElement {
  ...
  _filterNoaaWeakEchoes(imageData) {
    ...
    for (let index = 0; index < pixels.length; index += 4) {
      ...
      // NOAA's reflectivity palette uses gray/blue/cyan below the useful
      // precipitation range. Hide those weak echoes so visible rain starts
      // in green while yellow, red, magenta, and white storm returns remain.
  ...
  _apparentTemperature(attrs, units) {
    ...
    return value === undefined ? "" : this._displayTemp(value, units);
  }
  ...
  _icon(condition, size = 36) {
    ...
    if (c.includes("snow") && c.includes("rain")) return svg("ww-wintry", `${cloud()}<g class="ww-rain" stroke="url(#${iconId}-rain)" stroke-width="3.1" stroke-linecap="round"><path d="M18 47l-3 9M44 47l-3 9"/></g><g class="ww-snow">${snowflake(30, 52, .8)}</g>`);
    if (c.includes("snow") || c.includes("sleet")) return svg("ww-snowy", `${cloud()}<g class="ww-snow">${snowflake(20, 51, .72)}${snowflake(34, 55, .82)}${snowflake(48, 50, .68)}</g>`);
...
window.customCards.push({
  ...
  name: "RadarWise Weather (Lunar Test)",
  description: "Weather dashboard card with forecasts, theme support, and optional radar.",
  documentationURL: "https://github.com/ChrisandErin86/radar-wise-lunar-phase",
  preview: true
});
